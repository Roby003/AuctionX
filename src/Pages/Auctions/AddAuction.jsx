  ...
  const onDrop = useCallback((acceptedFiles) => {
    acceptedFiles.map((file) => {
      ...
      reader.onload = function (e) {
        setImages((prevState) => [...prevState, { id: cuid(), src: e.target.result, file: file }]);
      };
  ...
  const estimateTransactionCost = async () => {
    ...
    try {
      ...
      const latestBlocks = await Promise.all(
        [...Array(5)].map((_, i) => web3.eth.getBlock(parseInt(currentBlockNumber.toString()) - i))
      );

      const averageBaseFee =
        latestBlocks.reduce((acc, block) => acc + parseInt(block.baseFeePerGas), 0) / latestBlocks.length;
      const maxPriorityFeePerGas = web3.utils.toWei("2", "gwei");

      const gasEstimate = await auctionCreator.methods.createAuction(title, description, ["", "", ""]).estimateGas({
        from: accounts[0],
      });

      const estimateTransactionCostFunc = (gasEstimate, averageBaseFee, maxPriorityFeePerGas) => {
        try {
          /* eslint-disable */
          ...
          const totalCostWei = gasEstimateBigInt * averageBaseFeeBigInt + maxPriorityFeePerGasBigInt;

          const estimatedTransactionCost = web3.utils.fromWei(totalCostWei.toString(), "ether");
      ...
      return estimateTransactionCostFunc(gasEstimate, averageBaseFee, maxPriorityFeePerGas);
    } catch (error) {
    ...
    <Flex w={"100%"} className="test">
      {web3 && auctionCreator && (
        <Box w="100%" mx="auto" p="4">
          ...
          <GasCostModal isOpen={isOpen} onClose={onClose} clickHandler={clickHandler} gasEstimate={gasEstimateState} />
          <Box mb="4">
            ...
            <FormControl mt="5">
              ...
              <Input type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Name" mb="2" />
            </FormControl>
